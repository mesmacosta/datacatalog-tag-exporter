...
class TagManagerCLITest(unittest.TestCase):
    ...
    def test_parse_args_invalid_subcommand_should_raise_system_exit(self):
        self.assertRaises(SystemExit, datacatalog_tag_exporter_cli.DatacatalogTagExporterCLI._parse_args,
                          ['invalid-subcommand'])
    ...
    def test_run_no_args_should_raise_attribute_error(self):
        self.assertRaises(AttributeError, datacatalog_tag_exporter_cli.DatacatalogTagExporterCLI.run, None)

    @mock.patch(
        'datacatalog_tag_exporter.tag_datasource_exporter.TagDatasourceExporter')
