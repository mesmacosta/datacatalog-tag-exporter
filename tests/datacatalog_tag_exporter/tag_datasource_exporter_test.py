...
from datacatalog_tag_exporter import tag_datasource_exporter, constant


class TagDatasourceExporterTest(unittest.TestCase):

    @mock.patch('datacatalog_tag_exporter.datacatalog_facade.DataCatalogFacade')
    ...
    def test_export_tags_with_templates_filer_should_create_summary_file_and_template_file(self):
        tag_template_id = 'my_template'
        tag_template_id_2 = 'my_template_2'
    ...
    def test_export_tags_one_template_not_in_templates_filer_should_not_create_that_template_file(
        ...
        # Cleans up template files.
        os.remove(template_1_path)
        self.assertRaises(FileNotFoundError, os.remove, template_2_path)

        self.assertEqual(1, self.__datacatalog_facade.search_tag_templates.call_count)
        self.assertEqual(1, self.__datacatalog_facade.extract_resources_from_template.call_count)
        self.assertEqual(1, self.__datacatalog_facade.search_tagged_assets.call_count)
        self.assertEqual(1, self.__datacatalog_facade.list_tags.call_count)
        self.assertEqual(1, len(created_summary_file))
        first_row = created_summary_file.iloc[0]
    ...
    def test_export_tags_no_templates_in_templates_filer_should_not_create_template_and_summary_files(  # noqa: E501
        ...
        self.assertRaises(FileNotFoundError, pd.read_csv, self.__summary_file_path)

        template_1_path = os.path.join(self.__csv_file_path, '{}.csv'.format(tag_template_id))
        template_2_path = os.path.join(self.__csv_file_path, '{}.csv'.format(tag_template_id_2))

        # Cleans up template files.
        self.assertRaises(FileNotFoundError, os.remove, template_1_path)
        self.assertRaises(FileNotFoundError, os.remove, template_2_path)
